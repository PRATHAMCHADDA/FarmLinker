...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    // Optional CAPTCHA check — uncomment to enforce
    // if (!recaptchaToken) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#001f1f] via-[#0f332f] to-[#003c3c] p-6">
      ...
      >
        ...
        <input
          ...
          required
          className={`w-full p-3 rounded-xl mb-2 bg-white/5 ${
            passwordError ? 'border-red-400' : 'border-green-200/30'
          }`}
        />
        ...
        <input
          ...
          required
          className="w-full p-3 bg-white/5 border border-green-200/30 rounded-xl mb-6"
        />

        {/* CAPTCHA is optional right now */}
        <ReCAPTCHA
        ...
        <button
          type="submit"
          className="w-full bg-green-600 hover:bg-green-700 transition text-white py-3 rounded-xl font-semibold mt-4"
        >
          🌟 Create Account
        </button>
      </motion.form>
